...
- Using make:
  ...
  - You'll have a `server.out` and a `client.out` files in the project directory.
- Using cmake:
  - Create a bin directory `mkdir bin`.
  - Navigate to it `cd bin`.
  - Run `cmake .. && make`.
  - You'll have `server.out` and `client.out` in the bin directory.

Run the server using `./server.out [SERVER_PORT]`. The server will run on `localhost`.

Run the client using `./client.out [SERVER_IP] [SERVER_PORT]`.

## Running the Server with Docker
You can use Docker to build and run the server by just running:
```
